...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(
        isRateLimited ? err.message : "Invalid email or password.",
        "error",
      );
    } finally {
      setSubmitting(false);
    ...
    <div className="min-h-screen flex items-center justify-center bg-brown px-4">
      <div className="bg-cream rounded-sm p-8 md:p-10 w-full max-w-md">
        <div className="text-center mb-8">
          <span className="font-display italic text-2xl text-maroon">
            House of Jaee
          </span>
        ...
        <p className="text-xs text-brown-light text-center mt-6">
          For security, repeated failed attempts will temporarily lock login
          access.
        </p>
      </div>
    </div>
